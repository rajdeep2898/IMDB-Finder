...
module.exports = {
  checkMovie: async (ID, credentials) => {
    var ans;
    await axios
      .get(REQUEST_URL + ID)
      .then(async function (response) {
        ...
        let trailer =
          (await $(
            "#title-overview-widget > div.vital > div.slate_wrapper > div.videoPreview.videoPreview--autoPlaybackOnce > div.slate > a"
          ).attr("href")) ||
          $(
            "#titleVideoStrip > div.mediastrip_big > span:nth-child(1) > a"
          ).attr("href");
        ...
        let stars = [];
        $("div.plot_summary > div:nth-child(4)")
          .find("a")
          .each((i, str) => {
        ...
        const movie = {
          ...
          director:
            $(
              "#title-overview-widget > div.minPosterWithPlotSummaryHeight > div:nth-child(3) > div.plot_summary > div:nth-child(2) > a"
            )
              .text()
              .trim() ||
            $(
              "#title-overview-widget > div.minPosterWithPlotSummaryHeight > div:nth-child(2) > div.plot_summary > div:nth-child(2) > a"
            )
              .text()
              .trim() ||
            $(
